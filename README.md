...
- **Cartões de Crédito**: Gerencie múltiplos cartões com sub-abas individuais
- **Gerenciamento de Cartões**: Adicione, edite e remova cartões com limite e dia de fechamento
...
> ⚠️ **MIGRAÇÃO**: Se você já tem a tabela `transactions`, execute este SQL adicional:
> ```sql
> -- Criar tabela de cartões (se não existir)
> -- (execute o SQL acima para credit_cards)
> 
> -- Adicionar coluna card_id à tabela existente
> alter table public.transactions add column if not exists card_id bigint references public.credit_cards(id) on delete set null;
> create index if not exists idx_transactions_card_id on public.transactions(card_id);
> ```
